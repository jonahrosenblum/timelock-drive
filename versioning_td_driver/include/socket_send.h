...
/*
 * IPC transport mode flag.  Set to 1 before calling init_driver() to use
 * shared-memory instead of TCP sockets.  Declared here so all translation
 * units that include socket_send.h can read it.
 */
extern int g_use_ipc;

/*
 * Initialise the client side of the IPC shared-memory channel.
 * Must be called (when g_use_ipc == 1) before any send_message / gk_recv.
 * The gatekeeper server must already have created the shared-memory object.
 */
void ipc_transport_init_client(void);

/*
 * Receive exactly `len' bytes into `buf' from whichever transport is active
 * (TCP socket or shared-memory ring buffer).  Returns `len' on success,
 * -1 on error (TCP mode only; IPC mode always succeeds or blocks).
 */
ssize_t gk_recv(void *buf, size_t len);

/*
 * Send exactly `len' bytes from `buf' using the active transport.
 * In TCP mode calls send(conn_fd, ...).
 * In IPC mode writes into the client→server shared-memory pipe.
 * Returns `len' on success, -1 on error (TCP mode only).
 */
